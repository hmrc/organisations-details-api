...
class NumberOfEmployeesService @Inject() (
  ...
  def get(matchId: UUID, employeeCountRequest: NumberOfEmployeesRequest, scopes: Iterable[String])(implicit
    hc: HeaderCarrier,
    request: RequestHeader,
    ec: ExecutionContext
  ): Future[Option[Seq[NumberOfEmployeesResponse]]] = {
    ...
    resolve(matchId).flatMap { organisationMatch =>
      ...
      cacheService
        ...
        .map(x => x.references.map(y => y.map(NumberOfEmployeesResponse.create)))
    }
  }
}
