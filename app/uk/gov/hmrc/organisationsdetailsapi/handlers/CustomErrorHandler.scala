...
import uk.gov.hmrc.organisationsdetailsapi.errorhandler.ErrorResponses.{ErrorInvalidRequest, ErrorNotFound, ErrorPayloadRequired}
...
class CustomErrorHandler @Inject()(
  ...
  override def onClientError(request: RequestHeader, statusCode: Int, message: String): Future[Result] = {
    ...
    implicit val hc = HeaderCarrierConverter.fromRequestAndSession(request = request, session = request.session)
    ...
    statusCode match {
      ...
      case BAD_REQUEST if message.contains("No content to map") =>
        auditConnector.sendEvent(dataEvent("ServerValidationError", "Request bad format exception", request))
        Future.successful(ErrorPayloadRequired().toHttpResponse)
      case BAD_REQUEST =>
        auditConnector.sendEvent(dataEvent("ServerValidationError", "Request bad format exception", request))
