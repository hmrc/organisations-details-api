...
case class IfSelfAssessmentReturnDetails(utr: String, startDate: String, taxPayerType: String, taxSolvencyStatus: String, taxYears: Seq[IfTaxYear])

object IfSelfAssessmentReturnDetails {
  ...
  implicit val selfAssessmentResponseFormat: Reads[IfSelfAssessmentReturnDetails] = (
      (JsPath \ "utr").read[String](pattern(utrPattern, "UTR pattern is incorrect")) and
        ...
        (JsPath \ "taxyears").read[Seq[IfTaxYear]]
      )(IfSelfAssessmentReturnDetails.apply _)
}